...
def show_main_page():
    ...
    if notification_result.get("status") == "успіх":
        notifications = [n for n in notification_result.get("сповіщення", []) if not n.get("is_read")]
        if notifications:
            with st.expander(f"🔔 Нові сповіщення ({len(notifications)})", expanded=False):
                ...
                if st.button("✅ Позначити всі як прочитані"):
                    get_notifications(st.session_state.user["id"], mark_read=True)
                    st.success("Усі сповіщення позначено як прочитані.")
                    st.rerun()

    # Перехід у режим перегляду книги
    if st.session_state.show_book_details:
        show_book_details(st.session_state.show_book_details)
        return

    # Перехід у режим перегляду користувача
            ...
            with cols[i % 3]:
                with st.container():
                    ...
                    <div class="book-card">
                        ...
                        <p>Жанр: {book.get('genre_name') or 'Не вказано'}</p>
                        <p>Власник: {book.get('owner_name') or 'Невідомо'}</p>
...
def show_book_details(book_id):
    ...
    with col3:
        ...
        if cover_url:
            ...
            try:
                ...
                st.error(f"Не вдалося завантажити зображення: {e}")

        st.subheader("Власник книги")
        st.markdown(f"""
        <div class="card" style="line-height: 1.6;">
        <div><h4>{book.get('owner_name') or 'Невідомо'}</h4></div>
        <div>Регіон: {book.get('owner_region') or 'Не вказано'}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        elif book["status"] == "підтверджена":
            status_text = '<span class="book-status-borrowed">Підтверджена</span>'
        ...
        with st.expander(f"Відгуки ({len(reviews)})", expanded=False):
            if reviews:
    ...
    if 'edit_book' in st.session_state:
        ...
        with st.form(key="edit_book_form"):
            ...
            is_free = st.checkbox("Безкоштовно (не вимагає обміну)", value=book.get("is_free", False),
                                  key="edit_is_free")

            description = st.text_area("Опис книги", value=book.get("description", ""), key="edit_description")

            uploaded_cover = st.file_uploader("Завантажити фото (jpg, png)", type=["jpg", "jpeg", "png"],
                                              key="edit_cover")
            col1, col2 = st.columns(2)
            with col1:
                    ...
                    with st.spinner("Оновлення книги..."):
                        ...
                        book_data = {
                            ...
                            "genre_id": genre_id_map.get(
                                selected_genre) if selected_genre != "Не вказано" else None,
                            "is_free": int(is_free),
                        }
...
def show_my_books():
    ...
    with st.expander("📝 Додати нову книгу", expanded=False):
        with st.form(key="add_book_form"):
            ...
            col1, col2 = st.columns(2)
            with col1:
                title = st.text_input("Назва книги(обов'язково)", key="add_title")
            with col2:
                author = st.text_input("Автор(обов'язково)", key="add_author")
            ...
            if submit:
                if title and author:
                    with st.spinner("Додавання книги..."):
                        ...
                        if result.get("status") == "успіх":
                            st.success(f"Книгу успішно додано!")
                            time.sleep(2)
                            st.rerun()
                        else:
    ...
    if books_result.get("status") == "успіх":
        ...
        lent_books = [book for book in books if book["status"] == "підтверджена"]

        # Закладки для разных групп книг
        tabs = st.tabs(["Всі книги", "Доступні", "Зарезервовані", "Підтвердженні"])
        ...
        with tabs[3]:
            if lent_books:
                ...
                st.info("У вас немає підтверджених книг.")
    else:
    ...
    with tabs[0]:
        ...
        if result.get("status") == "успіх":
            ...
            if not exchanges:
                ...
                returned_requests = [ex for ex in exchanges if ex["status"] == "отримано"]
                ...
                col4.metric("Отримані", len(returned_requests))

                # Закладки для різних статусів
                status_tabs = st.tabs(["Очікують", "Активні", "Відхилені", "Отримані"])
                    ...
                    if accepted_requests:
                        for ex in accepted_requests:
                            ...
                            <div class="card">
                                ...
                                <p><strong>Позичальник:</strong> {ex['borrower_name']}</p>
                                <p><strong>Дата запиту:</strong> {format_date(ex['start_date'])}</p>
                                <p><strong>Статус:</strong> <span class="exchange-status-accepted">Підтверджено</span></p>
                            </div>
                            """, unsafe_allow_html=True)
                    ...
                    if returned_requests:
                        for ex in returned_requests:
                            ...
                            <div class="card">
                                ...
                                <p><strong>Позичальник:</strong> {ex['borrower_name']}</p>
                                <p><strong>Дата запиту:</strong> {format_date(ex['start_date'])}</p>
                                <p><strong>Дата отримання:</strong> {format_date(ex['end_date'])}</p>
                                <p><strong>Статус:</strong> <span class="exchange-status-returned">Отримано</span></p>
                            </div>
                            """, unsafe_allow_html=True)
                    else:
                    ...
                    if accepted_requests:
                        for ex in accepted_requests:
                            ...
                            <div class="card">
                                ...
                                <p><strong>Власник:</strong> {ex['owner_name']}</p>
                                <p><strong>Дата запиту:</strong> {format_date(ex['start_date'])}</p>
                                <p><strong>Статус:</strong> <span class="exchange-status-accepted">Активно</span></p>
                            </div>
                            """, unsafe_allow_html=True)

                            col1, col2 = st.columns(2)
                            with col1:
                                if st.button("Отримати книгу", key=f"return_{ex['id']}"):
                                    with st.spinner("Обробка запиту..."):
                                        result = update_exchange_status(ex['id'], "отримано")
                                        if result.get("status") == "успіх":
                                            st.success("Книгу позначено як отриману!")
                                            time.sleep(2)
                                            st.rerun()
                                        else:
                                            st.error(result.get("повідомлення", "Помилка при оновленні запиту"))
                                ...
                                if st.button("Оцінити користувача", key=f"review_o_{ex['owner_id']}"):
                                    with st.form(key=f"review_owner_form_{ex['owner_id']}"):
                                        ...
                                        comment = st.text_area("Коментар",
                                                               placeholder="Напишіть свій відгук про користувача")
                                        submit = st.form_submit_button("Додати відгук", use_container_width=True)
                                        ...
                                        if submit:
                                            with st.spinner("Додавання відгуку..."):
                                                ...
                                                if result.get("status") == "успіх":
                                                    ...
                                                    st.error(
                                                        result.get("повідомлення", "Помилка при додаванні відгуку"))
                            with col2:
                    ...
                    with st.spinner("Додавання в список бажаного..."):
                        ...
                        if result.get("status") == "успіх":
                            if result.get("знайдені_книги"):
                                st.success(
                                    f"Додано в список бажаного! Знайдено {len(result.get('знайдені_книги'))} відповідних книг.")
                            else:
...
def show_messages():
    ...
    if chat_list_result.get("status") != "успіх":
        st.error(chat_list_result.get("повідомлення", "Не вдалося отримати список контактів."))
        return
    ...
    contacts = [
        ...
        for contact in chat_list_result.get("контакти", [])
    ]

    # Перевірка, чи вибраний користувач є в списку
    ...
    if chat_user_id and all(c["id"] != chat_user_id for c in contacts):
        ...
        chat_user = chat_user_info.get("користувач")
        if chat_user:
            contacts.append({
                "id": chat_user_id,
    ...
    with col1:
        ...
        for contact in contacts:
            # Текст кнопки з ім'ям та кількістю непрочитаних повідомлень
            button_label = contact['username']
            unread = int(contact['unread_messages'])

            if unread > 0:
                button_label += f" ({unread} непрочитаних)"

            # Виділення активного контакту стилем кнопки
...
def show_user_profile(user_id):
    ...
    with col1:
        st.button("← Назад", on_click=reset_details)
    with col2:
        st.title(f"Профіль користувача")
    ...
    with col2:
        ...
        st.markdown(f"""
                <div class="card">
                    <h3>Інформація</h3>
                    <p><strong>Повне ім'я:</strong> {user.get("full_name") or "Не вказано"}</p>
...
def show_user_search():
    ...
    with col1:
        query = st.text_input("Ім'я користувача або повне ім'я", st.session_state.search_params.get('query', ''))
                    ...
                    for i, user in enumerate(users):
                        with cols[i % 3]:
                            ...
                            <div class="user-card" style="display: block;">
                                <h3>{user.get('username')}</h3>
                                <p>{user.get('full_name') or 'Імя не вказано'}</p>
                                <p><strong>Регіон:</strong> {user.get('region') or 'Не вказано'}</p>
                                <p>{display_rating(user.get('rating') or 0)}</p>
        ...
        if result.get("status") == "успіх":
            ...
            with col1:
                ...
                <div class="card" style="text-align: center;">
                    <h5>Всього обмінів</h5>
                    <p style="font-size: 1.5rem; font-weight: 600;">{stats.get("обмінів_всього", 0)}</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col2:
                st.markdown(f"""
                <div class="card" style="text-align: center;">
                    <h5>Успішних обмінів</h5>
                    <p style="font-size: 1.5rem; font-weight: 600;">{stats.get("обмінів_успішних", 0)}</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col1:
                ...
                with st.expander("Показати популярні жанри", expanded=False):
                    if popular_genres:
            ...
            with col2:
                ...
                with st.expander("Показати нові книги", expanded=False):
                    if newest_books:
            ...
            st.subheader("Користувачі з найбільшою кількістю книг")
